            ...
            if (topographicMap[currentPosition.first][currentPosition.second] == 0) {
                ...
                while (!travelledRoutes.empty())
                {
                    ...
                    if (topographicMap[std::get<0>(travelledRoutes.top()).first][std::get<0>(travelledRoutes.top()).second] == 9) {
                        visitedTopPoints.insert({std::get<0>(travelledRoutes.top()).first, std::get<0>(travelledRoutes.top()).second});
                        travelledRoutes.pop();
                        std::get<1>(travelledRoutes.top())++;
                ...
                sumOfPossibleRoutes+=visitedTopPoints.size();
            }
        }
    }

    std::cout << "Sum of Possible Routes: " << sumOfPossibleRoutes << std::endl;
